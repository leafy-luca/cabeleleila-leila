...
auth(); // Garantir que o cliente tenha acesso
...
if ($id_agendamento) {
    // Buscar os dados do agendamento
            $sql_agendamento = "SELECT A.id_agendamento, A.data_hora, S.nome_servico, C.nome, C.telefone, C.id_cliente, ASV.id_servico 
                                ...
                                WHERE A.id_agendamento = :id_agendamento AND C.id_cliente = :id_cliente";

    $stmt_agendamento = $pdo->prepare($sql_agendamento);
    $stmt_agendamento->bindParam(':id_agendamento', $id_agendamento);
    $stmt_agendamento->bindParam(':id_cliente', $_SESSION['id_cliente']); // Garantir que o cliente está vendo seu próprio agendamento
    $stmt_agendamento->execute();
    $agendamento = $stmt_agendamento->fetch(PDO::FETCH_ASSOC);
    
    if (!$agendamento) {
        // Se o agendamento não for encontrado ou o cliente tentar acessar o agendamento de outro, redireciona ou exibe uma mensagem de erro
        echo "<script>alert('Agendamento não encontrado ou você não tem permissão para visualizar este agendamento!'); window.location.href = 'agendamentos.php';</script>";
        exit();
    }
} else {
        ...
        <div class="row">
            <section class="container d-flex justify-content-center align-items-center">
                <div class="col-md-6 col-lg-4 text-center">
                    <h2 class="display-6 mt-4">Reagendar seu Agendamento</h2>
                    
                    <form method="POST" action="../private/reg_reagendamento.php" class="form-horizontal mb-4 mx-auto text-center">
