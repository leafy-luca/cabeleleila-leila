...
if (isset($_GET['id'])) {
    ...
    try {
        ...
        echo "<script>alert('Agendamento cancelado com sucesso!'); window.location.href = 'dashboard.php';</script>";

    } catch (Exception $e) {
        // Se algo der errado, realiza o rollback da transação
        $pdo->rollBack();
        echo "<script>alert('Erro ao cancelar agendamento. Tente novamente.'); window.location.href = 'dashboard.php';</script>";
    }
} else {
